  ...
  const toggleComplete = async (lessonId: string) => {
    ...
    try {
      const res = await fetch('/api/user/lessons/complete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`, 'x-action': isRemoving ? 'remove' : 'add' },
      ...
      <div className="min-h-screen bg-[#010409] flex items-center justify-center p-6 text-center">
        <div className="max-w-md glass p-12 rounded-[3rem] border border-white/5 space-y-8">
           ...
           <div className="space-y-3">
             ...
             <p className="text-slate-500 text-sm leading-relaxed">Tvoj račun je kreiran, ali administrator ga mora odobriti prije pristupa akademiji.</p>
           </div>
      ...
      case 'lessons': return (
        <div className="space-y-16 animate-in fade-in">
          <div className="text-center space-y-4 max-w-2xl mx-auto px-4">
            ...
            <p className="text-slate-500 text-sm">Sve video lekcije dolaze iz baze u realnom vremenu.</p>
          </div>
      ...
      case 'suppliers': return (
        <div className="space-y-16 animate-in fade-in">
          <div className="text-center max-w-2xl mx-auto">
            ...
            <p className="text-slate-500 text-sm">Provjereni kontakti iz baze podataka.</p>
          </div>
        ...
        <div className="space-y-12 animate-in fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div onClick={() => setActiveView('lessons')} className="glass p-10 rounded-[3.5rem] border border-white/5 hover:border-blue-500/30 transition-all cursor-pointer group">
               ...
               <p className="text-slate-500 text-sm">Sve video lekcije sinkronizirane iz baze.</p>
            </div>
            <div onClick={() => setActiveView('chat')} className="glass p-10 rounded-[3.5rem] border border-white/5 hover:border-purple-500/30 transition-all cursor-pointer group">
               ...
               <p className="text-slate-500 text-sm">Live Chat Hub za odobrene članove.</p>
            </div>
            <div onClick={() => setActiveView('suppliers')} className="glass p-10 rounded-[3.5rem] border border-white/5 hover:border-emerald-500/30 transition-all cursor-pointer group">
               ...
               <p className="text-slate-500 text-sm">Ekskluzivni kontakti iz baze.</p>
            </div>
          </div>
        </div>
      );
      ...
      <header className="sticky top-0 z-50 glass border-b border-white/5 bg-black/40">
        <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4">
            ...
            {user.role === 'admin' && (
              <button onClick={() => setActiveView('admin')} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest border border-white/10 ${activeView === 'admin' ? 'bg-white text-black' : 'text-white'}`}>Admin</button>
            )}
